...
/// one answers a *reads* question, where the id is all of the device a read is
/// filed under. Keeping them apart is what lets the fleet route be answered
/// from the store plus a membership list, and keeps the scope split
/// (`/v1/reads` reads what was written, `/v1/inventory` reads what was
...
pub struct DeviceReads {
    ...
    #[cfg_attr(feature = "openapi", schema(value_type = String))]
    pub device: DeviceId,
    /// The latest read of each field, ordered by field name.
    ///
...
#[cfg_attr(feature = "ts", derive(ts_rs::TS))]
#[cfg_attr(feature = "openapi", derive(utoipa::ToSchema))]
pub struct FleetReads {
    /// The devices on this page, ordered by id.
    pub devices: Vec<DeviceReads>,
...
/// Filters for the fleet reads route, deserialized from the query string,
/// e.g. `?fields=RUNNING_STATE,FIRMWARE&where=RUNNING_STATE:stopped&limit=50`.
///
/// Separate from [`ReadQuery`] rather than an extension of it, because the two
/// describe different questions. `ReadQuery` scopes *one series* — one field of
/// one device, over a time span — so it carries `start`/`end` and a singular
/// `field`. This one scopes *a set of devices at one instant*, so it carries no
/// span at all and every filter it does carry is plural. Folding them into one
/// struct would document four parameters on each route that the route ignores.
///
/// Every field is optional, and an omitted filter means "do not narrow on this
/// axis".
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
#[cfg_attr(feature = "ts", derive(ts_rs::TS))]
// `IntoParams` for the same reason `ReadQuery` derives it: the route documents
// the struct the handler actually deserializes, so a renamed parameter is
// renamed in the document rather than leaving the document describing one the
...
pub struct FleetQuery {
    ...
    /// Keep only devices whose reads satisfy every `FIELD:value` predicate,
    /// comma-separated — e.g. `RUNNING_STATE:stopped,FIRMWARE:2.11`.
    ///
    /// A predicate selects *devices*, not reads: a row that satisfies it still
    /// carries every field `fields` asked for, so
