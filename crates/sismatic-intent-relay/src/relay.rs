...
//! writings an operator submitted, so every failure is a distinct event with a
//! distinct writing id and is worth a line.
...
use sismatic_api_types::{BatchId, Intent, ReadingValue, Timestamp, WritingId, WritingRecord};
...
use sismatic_store::outbox::{Claim, DynWritingDrain, Outcome, WritingDrain};
...
impl RelayHandle {
    ...
    /// Cooperative, as `SyncHandle::shutdown` is: a claimed writing
...
pub fn spawn(registry: Arc<Registry>, drain: DynWritingDrain, cfg: RelayConfig) -> RelayHandle {
...
async fn dispatch_one(device: &Device, registry: &Registry, drain: &dyn WritingDrain) -> bool {
...
async fn dispatch_batch(
    ...
    match group.run(&instruction).await {
        ...
        Err(error) => {
            ...
            // recording.
            warn!(%batch, %error, "a group writing failed for at least one member");
...
async fn settle_all(
    ...
    outcome_for: impl Fn(&WritingRecord) -> Outcome,
) {
    for record in records {
...
async fn report(drain: &dyn WritingDrain, id: WritingId, outcome: Outcome) {
    if let Outcome::Failed(ref reason) = outcome {
        warn!(writing = %id, reason, "writing failed");
    }
    if let Err(err) = drain.settle(id, outcome, now()).await {
        warn!(%err, "could not record the outcome of a writing");
    }
}

/// Decide what to do with writings a previous process left `InFlight`.
///
/// Delivery to a device is at-least-once: a process that dies after
/// [`Device::run`] returns and before `settle` lands leaves a writing claimed
...
/// writing without a second attempt.
///
...
/// forever. A writing whose budget is exhausted lands in the dead-letter state
/// instead, where an operator can see it.
async fn recover(device: &Device, drain: &dyn WritingDrain) {
    let Ok(stranded) = drain.in_flight(device.id().to_string()).await else {
        warn!("could not read the writings left in flight by a previous process");
        return;
    };
    ...
    warn!(
        ...
        "writings were left in flight by a previous process"
    );
    ...
    for record in stranded {
        let outcome = match (&record.intent, observed) {
            ...
            // The effect the writing asked for exists, so the writing is done.
        ...
        if let Err(err) = drain.settle(record.id, outcome, now()).await {
            warn!(%err, "could not settle a writing left in flight");
        }
    }
}
