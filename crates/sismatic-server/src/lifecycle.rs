...
///
/// `Option<JoinHandle>` because a disabled sweep starts no task at all, rather
/// than one that never ticks — the same shape `sismatic_sync::spawn` gives a
/// field whose interval resolved to *never*, and for the same reason: what is
/// running should be what the logs say is running.
...
///
/// `interval` of `None` starts nothing. That is a supported deployment — a
/// store bounded by `max_memory` alone still cannot exhaust the machine — but
/// it is not a quiet one: without a sweep, history is kept until the budget
/// pushes it out, so `retain` describes nothing and the oldest reading a
/// dashboard can reach becomes a function of the poll schedule.
pub fn spawn(store: DynLifecycle, retain: Retention, interval: Option<Duration>) -> SweeperHandle {
    ...
    let Some(interval) = interval else {
        ...
        return SweeperHandle { task: None, cancel };
    };
    ...
    let task = tokio::spawn(sweep_loop(store, retain, interval, cancel.clone()));
    SweeperHandle {
        task: Some(task),
        cancel,
    }
...
/// idempotent, an immediate first pass on an empty store costs one lock.
...
async fn sweep_loop(
    ...
    // Re-pace from completion rather than firing a burst of catch-up ticks, for
    // the same reason the poll loops do: a sweep that ran long was competing for
    // the store's locks, and the answer to that is not to sweep more.
    ...
    loop {
        tokio::select! {
            ...
            _ = ticker.tick() => sweep(store.as_ref(), &retain, &mut seen_evicted).await,
        }
    }
