//! `/v1/reads/devices` — the fleet index, as a client meets it.
//!
...
use crate::{ANNEX, ATRIUM, GROUP, SCOPE, get, read_at};
...
/// Start the application over a store pre-loaded with `reads` and a stated
/// catalog; return its base URL.
async fn spawn(reads: impl IntoIterator<Item = Read>, catalog: MemoryCatalog) -> String {
...
/// A recording-state read, the field most of these filters are written against.
fn state(device: &str, value: RecordingState, at: &str) -> Read {
...
async fn the_whole_fleet_comes_back_one_row_per_device_ordered_by_id() {
    ...
    // Id order, not the order the catalog was written in or the reads were
...
#[tokio::test]
async fn a_configured_device_that_has_never_answered_is_a_row_with_no_reads() {
...
async fn fields_selects_which_columns_each_row_carries() {
    let address = spawn(
        [
            state(ATRIUM, RecordingState::Started, AT),
            read_at(ATRIUM, "FIRMWARE", ReadValue::Version("2.11".into()), AT),
            read_at(ATRIUM, "TIMEZONE", ReadValue::Text("UTC".into()), AT),
        ],
        fleet(),
    )
    .await;
...
async fn where_selects_devices_and_leaves_their_other_requested_fields_alone() {
    ...
    let address = spawn(
        [
            ...
            read_at(ATRIUM, "FIRMWARE", ReadValue::Version("2.11".into()), AT),
            state(ANNEX, RecordingState::Stopped, AT),
            read_at(ANNEX, "FIRMWARE", ReadValue::Version("2.09".into()), AT),
        ],
        fleet(),
    )
    .await;
...
async fn a_predicate_is_matched_in_the_shape_the_device_answered_in() {
    ...
    let address = spawn(
        [
            read_at(ATRIUM, "HTTP_PORT", ReadValue::Port(8080), AT),
            read_at(ANNEX, "HTTP_PORT", ReadValue::Port(9090), AT),
        ],
        fleet(),
    )
    .await;
...
async fn an_unknown_device_in_the_filter_is_a_404_that_names_it() {
    // The claim the catalog is entitled to make, and the reads routes below this
    // one are not. A silently smaller page would report a typo as a healthy
    // fleet.
    let address = spawn([], fleet()).await;
