//! The handlers, one module per route.
//!
//! Handlers are plain `async fn`s that take extractors and return a response;
//! nothing here knows how the application is assembled or what it is served on.
//! That is [`crate::startup`]'s job, and the split is what lets a route be read
//! (and reasoned about) without reading the wiring.

// Public like `reads`, and for one narrow reason: the `#[utoipa::path]`
// attribute on a handler expands to a sibling item in the handler's own module,
// and `crate::openapi` has to name it. Re-exporting the function is not enough —
// the generated item is not the function.
pub mod config;
pub mod devices;
pub mod error;
pub mod fleet_group_reads;
pub mod fleet_reads;
pub mod group_reads;
pub mod health_check;
pub mod instructions;
pub mod reads;
pub mod target;
pub mod writes;

pub use config::{patch_config, read_config, reload_config};

pub use devices::{
    add_device, add_group, export_config, list_devices, list_groups, read_device, read_group,
    remove_device, remove_group, replace_device, replace_group, reset_config,
};

pub use fleet_group_reads::list_fleet_groups;

pub use fleet_reads::list_fleet;

pub use instructions::{field_catalog, writes_catalog};

pub use group_reads::{group_field_history, list_group_fields, read_group_field};

pub use error::ApiFailure;
pub use health_check::health_check;
pub use reads::{field_history, list_fields, read_field};
pub use writes::{
    IdempotencyKey, ValueWrite, list_group_writes, list_writes, pause_group_recording,
    pause_recording, read_desired_recording_state, read_group_desired_recording_state, read_write,
    set_group_metadata, set_group_setting, set_metadata, set_setting, start_group_recording,
    start_recording, stop_group_recording, stop_recording,
};
